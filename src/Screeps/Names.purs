...
instance encodeRoomName :: EncodeJson RoomName where
  encodeJson (RoomName rn) = encodeJson rn

instance decodeRoomName :: DecodeJson RoomName where
  decodeJson = map RoomName <<< decodeJson
...
instance showCreepName :: Show CreepName where
  show (CreepName rn) = rn

instance encodeCreepName :: EncodeJson CreepName where
  encodeJson (CreepName rn) = encodeJson rn

instance decodeCreepName :: DecodeJson CreepName where
  decodeJson = map CreepName <<< decodeJson
